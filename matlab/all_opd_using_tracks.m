...
%%% trouble shooting line %%% pseudo_label_store = [];
opd = [];
area =[];
t_index = 0;
t_nonzero = 0;
...
for t = T;                                %%% 3rd iteration to load all images with selected frame number
      ...
      phase = imread(phase_file_name);
          ...
          pseudo_label = L(x,y);                            %%% coord switch for matrix data point retrieval, get the pseudolabel for cell_ID based on x1y1 coordinate value in pseudo image L, psuedo b/c it changes each iteration

          %%%troubleshooting line%%%  pseudo_label_store = [pseudo_label_store, pseudo_label];
        %%%%%%%%%% put an else statement in here to record a 0 value for opd if pseudo_label = 0

if (pseudo_label == 0)
  results.opd(cell_ID_final,t_nonzero) = 0;
  results.area(cell_ID_final,t_nonzero) = 0;
      else
